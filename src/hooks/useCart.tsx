...
export function CartProvider({ children }: CartProviderProps): JSX.Element {
  ...
  const cartPreviuosValue = prevCarRef.current ?? cart;
  ...
  const addProduct = async (productId: number) => {
    try {
      // const productAdd = (await api.get(`products/${productId}`)).data;
      // localStorage.setItem("@RocketShoes:cart", JSON.stringify(productAdd));
  ...
  const updateProductAmount = async ({
    ...
    try {
      ...
      if (amount > stockAmount) {
        toast.error('Quantiade solicitada fora de estoque');
        return;
      }
